 ...
 * invocation time. _Passive_ means, that it has to be `invoke`d by an external
 * caller with the current time. Only when the time span between the current
 * time and the last invocation exceeds the specified `interval`, the `callback`
 ...
 *
 * TimerGroup simplifies the handling of multiple Timer objects by offering a
 * unifying Timer-like interface. Additionally, it serves as a barrier and
 * prevents timers from being invoked too often by defining a minimum invocation
 * interval (MI). All registered timer intervals should be larger than MI and
 * also be multiples of MI. Similar to Timer, a TimerGroup is _passive_ and
 * needs to be `invoke`d by an external caller.
 */
class TimerGroup {
/// Registered timers.
...
public:
/// \param minInterval The minimum interval between invocations of registered
/// timers.
