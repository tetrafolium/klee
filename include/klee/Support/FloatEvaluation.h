...
inline uint64_t add(uint64_t l, uint64_t r, unsigned inWidth) {
	...
	case FLT_BITS:
		return bits64::truncateToNBits(
			FloatAsUInt64(UInt64AsFloat(l) + UInt64AsFloat(r)), FLT_BITS);
	case DBL_BITS:
		return bits64::truncateToNBits(
			DoubleAsUInt64(UInt64AsDouble(l) + UInt64AsDouble(r)), DBL_BITS);
	default:
		llvm::report_fatal_error("unsupported floating point width");
	}
}
...
inline uint64_t mod(uint64_t l, uint64_t r, unsigned inWidth) {
	...
	case FLT_BITS:
		return bits64::truncateToNBits(
			FloatAsUInt64(fmod(UInt64AsFloat(l), UInt64AsFloat(r))), FLT_BITS);
	case DBL_BITS:
		return bits64::truncateToNBits(
			DoubleAsUInt64(fmod(UInt64AsDouble(l), UInt64AsDouble(r))), DBL_BITS);
	default:
		llvm::report_fatal_error("unsupported floating point width");
	}
}
...
inline uint64_t SignedIntToFP(uint64_t l, unsigned outWidth, unsigned inWidth) {
	...
	case FLT_BITS:
		return bits64::truncateToNBits(
			FloatAsUInt64((float)(int64_t)ints::sext(l, 64, inWidth)), outWidth);
	case DBL_BITS:
		return bits64::truncateToNBits(
			DoubleAsUInt64((double)(int64_t)ints::sext(l, 64, inWidth)), outWidth);
	default:
		llvm::report_fatal_error("unsupported floating point width");
	}
}
